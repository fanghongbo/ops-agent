...
func DiskIOMetrics() (L []*model.MetricValue) {
	...
	dsList, err = nux.ListDiskStats()
	if err != nil {
		dlog.Errorf("get disk status list err: %s", err)
		return
	}
...
func IOStatsMetrics() (L []*model.MetricValue) {
	...
	for device := range diskStatsMap {
		...
		deltaIoRS := IODelta(device, IOReadSectors)
		deltaIoWS := IODelta(device, IOWriteSectors)
		ioMR := IODelta(device, IOMsecRead)
		ioMW := IODelta(device, IOMsecWrite)
		...
		svcTm := 0.0
		if nIo != 0 {
			avgRqSz = float64(deltaIoRS+deltaIoWS) / float64(nIo)
			await = float64(ioMR+ioMW) / float64(nIo)
			svcTm = float64(use) / float64(nIo)
		}

		duration := IODelta(device, TS)

		L = append(L, GaugeValue("disk.io.read_bytes", float64(deltaIoRS)*512.0, tags))
		L = append(L, GaugeValue("disk.io.write_bytes", float64(deltaIoWS)*512.0, tags))
		L = append(L, GaugeValue("disk.io.avgrq_sz", avgRqSz, tags))
...
func IOStatsForPage() (L [][]string) {
	...
	for device := range diskStatsMap {
		...
		item := []string{
			...
			fmt.Sprintf("%d", rio),
			fmt.Sprintf("%d", wio),
			fmt.Sprintf("%.2f", float64(deltaIoRS)/2.0),
			fmt.Sprintf("%.2f", float64(deltaIoWS)/2.0),
			fmt.Sprintf("%.2f", avgRqSz),                                              // avgrq-sz: delta(rsect+wsect)/delta(rio+wio)
